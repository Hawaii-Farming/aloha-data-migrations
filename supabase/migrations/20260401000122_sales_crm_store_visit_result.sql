CREATE TABLE IF NOT EXISTS sales_crm_store_visit_result (
    ...
    CONSTRAINT chk_sales_crm_visit_result_product CHECK (
        (sales_product_id IS NOT NULL AND sales_crm_external_product_id IS NULL)
        OR (sales_product_id IS NULL AND sales_crm_external_product_id IS NOT NULL)
    )
);
...
CREATE INDEX idx_sales_crm_visit_result_ext ON sales_crm_store_visit_result (sales_crm_external_product_id);
