CREATE TABLE IF NOT EXISTS org_module (
    ...
    PRIMARY KEY (org_id, sys_module_id)
);

COMMENT ON TABLE org_module IS 'Org-scoped copy of system modules. Seeded when a new org is created. Org admins toggle is_enabled to control which modules are available to their users. Composite PK (org_id, sys_module_id) lets every org reuse the same canonical sys_module ids without ID-namespace collisions.';
