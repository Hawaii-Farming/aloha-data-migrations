...
CREATE UNIQUE INDEX uq_org_site_org_level ON org_site (org_id, name) WHERE farm_name IS NULL;
CREATE UNIQUE INDEX uq_org_site_farm_level ON org_site (org_id, farm_name, name) WHERE farm_name IS NOT NULL;

CREATE INDEX idx_org_site_org_id ON org_site (org_id);
CREATE INDEX idx_org_site_farm ON org_site (farm_name);
...
COMMENT ON COLUMN org_site.farm_name IS 'Inherited from parent org_farm when site is farm-scoped; null for org-wide sites';
