CREATE TABLE IF NOT EXISTS grow_fertigation_recipe_item (
    ...
    application_uom             TEXT NOT NULL REFERENCES sys_uom(id),
    ...
    CONSTRAINT grow_fertigation_recipe_item_farm_fkey FOREIGN KEY (org_id, farm_id) REFERENCES org_farm(org_id, id)
);
...
COMMENT ON COLUMN grow_fertigation_recipe_item.item_name IS 'Pre-filled from invnt_item.id when invnt_item_id is set; editable';
