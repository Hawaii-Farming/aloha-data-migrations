CREATE TABLE IF NOT EXISTS grow_harvest_weight (
    ...
    grow_harvest_container_name   TEXT NOT NULL REFERENCES grow_harvest_container(name),
...
COMMENT ON COLUMN grow_harvest_weight.grow_lettuce_seed_batch_id IS 'The lettuce seeding batch being harvested. Populated when farm_name = lettuce; null for cuke';
COMMENT ON COLUMN grow_harvest_weight.grow_cuke_seed_batch_id IS 'The cuke seeding batch being harvested. Populated when farm_name = cuke; null for lettuce';
COMMENT ON COLUMN grow_harvest_weight.grow_grade_id IS 'Grade assigned to this harvest (e.g. Grade A, Grade B)';
COMMENT ON COLUMN grow_harvest_weight.grow_harvest_container_name IS 'Container type used for this weigh-in; drives tare weight calculation';
...
CREATE INDEX idx_grow_harvest_weight_container ON grow_harvest_weight (grow_harvest_container_name);
