...
COMMENT ON TABLE org_site_gh_row IS 'Physical greenhouse row infrastructure. One row per physical GH row — pure identity (site_id, row_num). Crop-agnostic and rendering-agnostic. Referenced by seeding, scouting, maintenance, and spraying activities when they target a specific row. Bag counts and planting state live on grow_cuke_gh_row_planting (per scenario). Block membership and render order are defined in org_site_gh_block.';
