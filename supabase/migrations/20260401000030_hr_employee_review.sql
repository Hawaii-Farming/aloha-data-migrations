CREATE TABLE IF NOT EXISTS hr_employee_review (
    ...
    created_by          TEXT REFERENCES hr_employee(name),
    updated_by          TEXT REFERENCES hr_employee(name),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    is_deleted          BOOLEAN NOT NULL DEFAULT false,

    CONSTRAINT uq_hr_employee_review_quarter
        UNIQUE (org_id, hr_employee_name, review_year, review_quarter),
    ...
    CONSTRAINT fk_hr_employee_review_lead
        FOREIGN KEY (lead_name) REFERENCES hr_employee(name)
);

-- RLS lives in 20260401000200_sys_rls_policies.sql.

CREATE INDEX idx_hr_employee_review_org ON hr_employee_review (org_id);
CREATE INDEX idx_hr_employee_review_employee ON hr_employee_review (hr_employee_name);
