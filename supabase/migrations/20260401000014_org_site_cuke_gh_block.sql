CREATE TABLE IF NOT EXISTS org_site_cuke_gh_block (
    ...
    site_id         TEXT NOT NULL REFERENCES org_site_cuke_gh(site_id),
    ...
    CONSTRAINT uq_org_site_cuke_gh_block_site_block UNIQUE (site_id, block_num)
);
...
COMMENT ON COLUMN org_site_cuke_gh_block.block_num IS 'Block sequence (1, 2, 3...). The dashboard renders blocks in ascending block_num, with sidewalks between them';
...
COMMENT ON COLUMN org_site_cuke_gh_block.row_num_from IS 'First row_num in this block (inclusive). Block membership is defined by row_num range: a row belongs to the block where row_num_from <= row_num <= row_num_to';
COMMENT ON COLUMN org_site_cuke_gh_block.row_num_to IS 'Last row_num in this block (inclusive)';
COMMENT ON COLUMN org_site_cuke_gh_block.direction IS 'forward = rows render in ascending row_num order within the block. reverse = rows render in descending row_num order';
