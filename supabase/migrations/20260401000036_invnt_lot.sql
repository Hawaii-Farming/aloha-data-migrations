CREATE TABLE IF NOT EXISTS invnt_lot (
    ...
    CONSTRAINT uq_invnt_lot UNIQUE (org_id, invnt_item_name, lot_number)
);

COMMENT ON TABLE invnt_lot IS 'Tracks unique inventory lots by item and lot number. The id (PK) includes the item to ensure global uniqueness since different items can share the same lot number. The constraint on (org_id, invnt_item_name, lot_number) prevents duplicate lots per item.';
