CREATE TABLE IF NOT EXISTS sales_po_line (
    ...
    -- even if sales_sps_product_buyer_part is later edited. NULL for manual
    -- (non-EDI) orders.
...
COMMENT ON COLUMN sales_po_line.buyer_part_number IS 'EDI-only. Snapshot from 850 LineItem BuyerPartNumber. Resolved against sales_sps_product_buyer_part to set sales_product_id at PO receipt; preserved here so outbound 856/810 echo the original.';
...
COMMENT ON COLUMN sales_po_line.gtin_case IS 'EDI-only. Case-level GTIN-14 snapshot at PO receipt. Pulled from sales_sps_product_buyer_part.gtin_case; copied here so outbound 856/810 don''t depend on the lookup row still existing.';
