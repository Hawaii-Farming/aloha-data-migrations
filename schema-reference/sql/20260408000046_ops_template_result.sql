CREATE TABLE IF NOT EXISTS ops_template_result (
    ...
    ops_template_question_id         UUID        NOT NULL REFERENCES ops_template_question(id),
...
-- Site-scoped responses (the common case) are unique per (tracker, question).
-- Equipment-scoped responses (calibration logs, where one tracker contains
-- multiple readings of the same question against different equipment) are
-- unique per (tracker, question, equipment).
CREATE UNIQUE INDEX uq_ops_template_result_checklist_site
    ON ops_template_result (ops_task_tracker_id, ops_template_question_id)
    WHERE equipment_id IS NULL;
CREATE UNIQUE INDEX uq_ops_template_result_checklist_equipment
    ON ops_template_result (ops_task_tracker_id, ops_template_question_id, equipment_id)
...
COMMENT ON COLUMN ops_template_result.ops_template_question_id IS 'Sourced from ops_template_question';
