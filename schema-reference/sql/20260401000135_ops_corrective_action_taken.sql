...
COMMENT ON TABLE ops_corrective_action_taken IS 'Corrective actions raised against a failing checklist response, EMP test result, or pest trap inspection. Tracks the action required, who is responsible, and the resolution status.';
...
CREATE INDEX idx_ops_corrective_action_taken_pest     ON ops_corrective_action_taken (fsafe_pest_result_id);
...
COMMENT ON COLUMN ops_corrective_action_taken.fsafe_pest_result_id IS 'Sourced from the failing fsafe_pest_result that triggered this corrective action';
