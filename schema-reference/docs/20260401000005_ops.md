...
| ops_template_question_id      | UUID         | FK → ops_template_question(id), nullable          | Sourced from ops_template_question; null for ATP surface test results |
...
> `fsafe_result_id` and `ops_template_result_id` are mutually exclusive — exactly one is set per row.
