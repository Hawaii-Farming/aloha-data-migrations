...
erDiagram
    ...
    invnt_lot ||--o{ grow_seed_batch : lot_tracked
...
| org_id | TEXT | NOT NULL, FK → org(id) | |
| farm_id | TEXT | NOT NULL, FK → org_farm(id) | |
| site_id | TEXT | FK → org_site(id), nullable | Growing site being harvested; pre-filled from grow_seed_batch.site_id |
| ops_task_tracker_id | UUID | FK → ops_task_tracker(id), nullable | |
| grow_seed_batch_id | UUID | NOT NULL, FK → grow_seed_batch(id) | The seeding batch being harvested; filtered to status IN (seeded, transplanted, harvesting) |
