...
| org_id | TEXT | NOT NULL, FK → org(id) | |
| farm_id | TEXT | NOT NULL, FK → org_farm(id) | |
| grow_lettuce_seed_mix_id | TEXT | NOT NULL, FK → grow_lettuce_seed_mix(id) | |
...
Unique constraint on `(grow_lettuce_seed_mix_id, invnt_item_id)`.
...
| grow_lettuce_seed_mix_id | TEXT | FK → grow_lettuce_seed_mix(id), nullable | Set when seeding a mix; null when seeding a single variety. Mutually exclusive with invnt_item_id |
| invnt_item_id | TEXT | FK → invnt_item(id), nullable | Set when seeding a single seed item; null when seeding a mix. Mutually exclusive with grow_lettuce_seed_mix_id |
...
CHECK constraint: exactly one of `invnt_item_id` or `grow_lettuce_seed_mix_id` must be set.
