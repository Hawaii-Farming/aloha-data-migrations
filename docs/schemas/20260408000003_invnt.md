...
| equipment_name             | TEXT         | FK → org_equipment(name), nullable          | |
| invnt_vendor_name          | TEXT         | FK → invnt_vendor(name), nullable       | |
| manufacturer             | TEXT         | nullable                              | |
| grow_variety_id          | TEXT         | FK → grow_variety(code), nullable       | |
...
| id             | TEXT    | PK                                | |
| org_id         | TEXT    | NOT NULL, FK → org(id)            | |
| farm_name        | TEXT    | NOT NULL, FK → org_farm(name)       | Inherited from invnt_item.farm_name when lot is created |
...
Unique constraint on `(org_id, invnt_item_name, lot_number)`.

---

## invnt_po

Tracks purchase order requests through a workflow from request to receipt. Each order snapshots the item name, units, and cost at order time so the record stays accurate even if the item changes later. `request_type` determines whether it's a `non_inventory_item` purchase (classified by `invnt_category_id`) or an `inventory_item` purchase (linked via `invnt_item_name`). `item_name` is always populated either way. 
| Column                | Type         | Constraints                           | Description                              |
|----------------------|--------------|---------------------------------------|------------------------------------------|
...
| invnt_vendor_name      | TEXT         | FK → invnt_vendor(name), nullable       | Pre-filled from invnt_item.invnt_vendor_name when item is selected; editable |
...
| ordered_by           | TEXT         | FK → hr_employee(name), nullable        | |
...
| id                   | UUID         | PK, auto-generated                    | |
| org_id               | TEXT         | NOT NULL, FK → org(id)                | |
| farm_name              | TEXT         | FK → org_farm(name), nullable               | Inherited from invnt_po.farm_name when receiving against a PO |
...
| id                   | UUID         | PK, auto-generated                    | |
| org_id               | TEXT         | NOT NULL, FK → org(id)                | |
| farm_name              | TEXT         | FK → org_farm(name), nullable               | Inherited from invnt_item.farm_name when on-hand record is created |
| invnt_item_name        | TEXT         | NOT NULL, FK → invnt_item(name)         | |
...
| farm_name                      | invnt_item.farm_name                  | Optional farm scope                      |
| invnt_item_name                | invnt_item.name                       | The item                                 |
...
| invnt_vendor_name              | invnt_item.invnt_vendor_name          | Primary vendor                           |
