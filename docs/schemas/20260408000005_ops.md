...
| farm_name    | TEXT         | FK → org_farm(name), nullable        | Optional farm scope; NULL if task applies to all farms |
| name       | TEXT         | PK                       | |
...
| farm_name           | TEXT         | FK → org_farm(name), nullable           | Pre-filled from ops_task.farm_name when task is selected; editable |
| site_id           | TEXT         | FK → org_site(id), nullable           | |
| sales_product_id  | TEXT         | FK → sales_product(code), nullable      | The product being packed; set for packing activities, null for all other task types |
| ops_task_name       | TEXT         | NOT NULL, FK → ops_task(name)       | |
...
| verified_by       | TEXT         | FK → hr_employee(name), nullable    | |

---

## ops_task_schedule

Employee task assignments for both planning and execution. When `ops_task_tracker_id` is null, the row is a planned schedule entry. When set, it is an executed activity. `ops_task_name` is always set — derived from the tracker when linked, or selected by the user for planned entries.

| Column                 | Type         | Constraints                               | Description                              |
|-----------------------|--------------|------------------------------------------|------------------------------------------|
| id                    | UUID         | PK, auto-generated                       | |
| org_id                | TEXT         | NOT NULL, FK → org(id)                   | |
| farm_name               | TEXT         | FK → org_farm(name), nullable              | Inherited from ops_task_tracker.farm_name when linked to a tracker; user-selected for planned entries |
| ops_task_name           | TEXT         | NOT NULL, FK → ops_task(name)              | Inherited from ops_task_tracker.ops_task_name when linked to a tracker; user-selected for planned entries |
| ops_task_tracker_id   | UUID         | FK → ops_task_tracker(id), nullable      | |
| hr_employee_name        | TEXT         | NOT NULL, FK → hr_employee(name)           | |
...
Partial unique indexes: `(ops_task_tracker_id, hr_employee_name)` for executed entries; `(ops_task_name, hr_employee_name, start_time)` for planned entries.
...
| hr_employee_name          | TEXT         | Employee identifier                                                         |
| org_id                  | TEXT         | Organization                                                                |
| hr_department_name        | TEXT         | Employee department identifier                                              |
| hr_work_authorization_name| TEXT         | Employee work authorization identifier                                      |
...
| ops_training_type_name   | TEXT         | FK → ops_training_type(name), nullable     | |
...
| trainer_name             | TEXT         | FK → hr_employee(name), nullable           | Sourced from hr_employee; the employee who conducted the training session |
...
| id                      | UUID         | PK, auto-generated                    | |
| org_id                  | TEXT         | NOT NULL, FK → org(id)                | |
| farm_name                 | TEXT         | FK → org_farm(name), nullable               | Inherited from ops_training.farm_name when attendee record is created |
...
Unique constraint on `(ops_training_id, hr_employee_name)`.
...
|--------------------------|--------------|----------------------------------------------|------------------------------------------|
| org_id                   | TEXT         | NOT NULL, FK → org(id)                       | Owning organization for RLS filtering    |
| farm_name                  | TEXT         | FK → org_farm(name), nullable                      | Optional farm scope; null if the template applies to all farms |
| name                     | TEXT         | PK                                     | |
| org_module_name            | TEXT         | FK → org_module(name), nullable                | |
| description              | TEXT         | nullable                                     | |
| display_order            | INTEGER      | NOT NULL, default 0                          | |

Partial unique indexes: `(org_id, name)` where `farm_name IS NULL` for org-level templates; `(org_id, farm_name, name)` where `farm_name IS NOT NULL` for farm-level templates.
...
| org_id | TEXT | NOT NULL, FK → org(id) | |
| farm_name | TEXT | FK → org_farm(name), nullable | Inherited from ops_task.farm_name or ops_template.farm_name when the link is created |
| ops_task_name | TEXT | NOT NULL, FK → ops_task(name) | |
| ops_template_name | TEXT | NOT NULL, FK → ops_template(name) | |

Unique constraint on `(ops_task_name, ops_template_name)`.
...
| id                                 | UUID         | PK, auto-generated                    | |
| org_id                             | TEXT         | NOT NULL, FK → org(id)                | |
| farm_name                            | TEXT         | FK → org_farm(name), nullable               | Inherited from ops_template.farm_name when question is created |
| ops_template_name                    | TEXT         | NOT NULL, FK → ops_template(name)       | |
...
| id                   | UUID         | PK, auto-generated                       | |
| org_id               | TEXT         | NOT NULL, FK → org(id)                   | |
| farm_name              | TEXT         | FK → org_farm(name), nullable                  | Inherited from ops_task_tracker.farm_name when response is created |
| ops_task_tracker_id  | UUID         | NOT NULL, FK → ops_task_tracker(id)      | |
| ops_template_name      | TEXT         | NOT NULL, FK → ops_template(name)          | Sourced from ops_task_template; identifies which template this response belongs to |
...
| equipment_name         | TEXT         | FK → org_equipment(name), nullable             | The equipment this checklist was completed for; null for site-specific checklists |
...
- **Site-scoped responses** (the common case) are unique per `(ops_task_tracker_id, ops_template_question_id)` where `equipment_name IS NULL`.
- **Equipment-scoped responses** (e.g. calibration logs where one tracker contains multiple readings of the same question against different equipment) are unique per `(ops_task_tracker_id, ops_template_question_id, equipment_name)` where `equipment_name IS NOT NULL`.
...
| org_id                             | TEXT         | NOT NULL, FK → org(id)                              | |
| farm_name                            | TEXT         | FK → org_farm(name), nullable                             | |
| ops_template_name                    | TEXT         | FK → ops_template(name), nullable                     | Inherited from ops_template_result.ops_template_name when sourced from a failing checklist response |
...
| ops_corrective_action_choice_name    | TEXT         | FK → ops_corrective_action_choice(name), nullable     | |
| other_action                       | TEXT         | nullable                                            | |
| assigned_to                        | TEXT         | FK → hr_employee(name), nullable                      | |
