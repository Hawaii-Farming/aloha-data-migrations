...
| num_bags_capacity | INTEGER | NOT NULL | Maximum number of grow bags that physically fit in this row. Rows are always planted to full capacity. When a row is split between two varieties, each variety occupies capacity/2 bags |
| notes | TEXT | nullable | |
