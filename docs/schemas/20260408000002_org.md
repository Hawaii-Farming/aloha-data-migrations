...
Housing facility registry. One row per residence owned or managed by the organization. **Standalone**: `id` is the display name verbatim (e.g. `"BIP (5)"`, `"South Kohala"`) and is NOT FK-linked to `org_site`. Org-scoped — no farm linkage.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | TEXT | PK | Display name verbatim, including spaces and parens |
| org_id | TEXT | NOT NULL, FK → org(id) | |
...
| id | UUID | PK, default gen_random_uuid() | |
| org_id | TEXT | NOT NULL, FK → org(id) | |
| housing_id | TEXT | NOT NULL, FK → org_site_housing(id) | Which housing facility this area belongs to |
| name | TEXT | NOT NULL | Display label for the area (e.g. "Room 2A", "East Wing") |

Unique constraint on `(housing_id, name)`.
