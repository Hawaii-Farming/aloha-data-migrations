...
| farm_name | TEXT | NOT NULL, FK → org_farm(name) | Farm (crop line) this pack lot belongs to |
...
|--------|------|-------------|-------------|
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| farm_name | TEXT | FK → org_farm(name), nullable | Optional farm scope; null if the check applies to all farms |
| name | TEXT | PK | |
...
Partial unique indexes on `(org_id, name)` where `farm_name IS NULL` and `(org_id, farm_name, name)` where `farm_name IS NOT NULL`.
...
| invnt_item_name | TEXT | FK → invnt_item(name), nullable | Pre-filled from sales_product.invnt_item_name; filtered to packaging items in inventory |
...
| pack_shelf_life_metric_name | TEXT | NOT NULL, FK → pack_shelf_life_metric(name) | |
...
Unique constraint on `(pack_shelf_life_id, pack_shelf_life_metric_name, observation_date)` — one response per check per date per trial.
...
| pack_productivity_fail_category_name | TEXT | NOT NULL, FK → pack_productivity_fail_category(name) | |
| fail_count | INTEGER | NOT NULL, default 0 | Number of fails for this category in this hour |
| notes | TEXT | nullable | |

Unique constraint on `(pack_productivity_hour_id, pack_productivity_fail_category_name)`.
