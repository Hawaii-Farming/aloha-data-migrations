...
Org-specific departments used to classify employees. Each org defines its own set of departments. `name` is the PK and holds the display value (e.g. `"GH"`, `"PH"`, `"Lettuce"`).
...
Org-specific work authorization types used to classify employees. Each org defines its own set of types. `name` is the PK and holds the display value (e.g. `"Local"`, `"FUERTE"`, `"WFE"`, `"H1B"`).
...
Unified employee register and org membership table. Every employee gets a row here with a required `sys_access_level_name` that defines their role (owner, manager, team_lead, employee). Employees without app access have a null `user_id`. A user can belong to multiple orgs by having one row per org. Tracks employment details, management hierarchy, and compensation.
...
| hr_department_name         | TEXT         | FK → hr_department(name), nullable         | |
| sys_access_level_name      | TEXT         | NOT NULL, FK → sys_access_level(name)      | Sourced from sys_access_level; determines the employee role and module visibility |
| team_lead_name             | TEXT         | FK → hr_employee(name), nullable           | Filtered to employees with sys_access_level_name = team_lead |
| compensation_manager_name  | TEXT         | FK → hr_employee(name), nullable           | Filtered to employees with sys_access_level_name = manager |
| hr_work_authorization_name | TEXT         | FK → hr_work_authorization(name), nullable | |
...
| site_id                  | TEXT         | FK → org_site_housing(name), nullable          | References org_site_housing; the housing facility the employee is assigned to. Null if the employee is not housed |
...
| org_module_name | TEXT | NOT NULL, FK → org_module(name) | Sourced from org_module; identifies which module this access record controls |
...
Unique constraint on `(hr_employee_name, org_module_name)` — one record per employee per module.
...
| requested_by    | TEXT         | NOT NULL, FK → hr_employee(name)    | Auto-set to the logged-in employee when the request is created |
| reviewed_at     | TIMESTAMPTZ  | nullable                          | |
| reviewed_by     | TEXT         | FK → hr_employee(name), nullable    | |
...
| reported_by                    | TEXT         | FK → hr_employee(name), nullable    | |
| reviewed_at                    | TIMESTAMPTZ  | nullable                          | |
| reviewed_by                    | TEXT         | FK → hr_employee(name), nullable    | |
...
| employee_name | TEXT | NOT NULL | Name as it appears in the payroll processor data; hr_employee_name is matched by the import script |
| hr_department_name | TEXT | FK → hr_department(name), nullable | Snapshot from hr_employee.hr_department_name at time of import |
| hr_work_authorization_name | TEXT | FK → hr_work_authorization(name), nullable | Snapshot from hr_employee.hr_work_authorization_name at time of import |
...
| module_id | TEXT | org_module.name | |
| module_slug | TEXT | sys_module.name | Human-readable slug (e.g. `hr`, `grow`, `pack`) |
...
| sub_module_id | TEXT | org_sub_module.name | |
| sub_module_slug | TEXT | sys_sub_module.name | |
...
hr_employee e
  ...
  ↳ org_sub_module osm              (scoped to employee's org)
       ↳ org_module om              (same org + sys_module_name)
