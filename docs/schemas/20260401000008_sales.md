...
QuickBooks invoice line items. Nightly-synced from the invoices spreadsheet today (`invoices_23-25`, `invoices_2025` tabs); moving to direct QB API later, at which point QB line-item numbers will be included and enable unique-ness. One row per line item — a single `invoice_number` can appear across multiple rows with different `product_code` / `variety` / `grade` combinations.
...
No uniqueness constraint pending QB line-item numbers.

### sales_invoice_v (view)

`SELECT * FROM sales_invoice WHERE is_deleted = false` plus derived columns: `year`, `month`, `iso_year`, `iso_week`, `dow` (all `INT`, extracted from `invoice_date`). Dashboards should query this view rather than the underlying table.
