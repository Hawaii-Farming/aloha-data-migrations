...
| farm_name                     | TEXT         | NOT NULL, FK → org_farm(name)            | Farm (crop line) this product belongs to |
| grow_grade_id                    | TEXT         | FK → grow_grade(code), nullable      | |
| code                        | TEXT         | PK                           | |
| name                        | TEXT         | NOT NULL                           | |
| description                 | TEXT         | nullable                           | |
| invnt_item_name          | TEXT         | FK → invnt_item(name), nullable      | Filtered to packaging items in inventory |
...
| sales_product_id  | TEXT        | NOT NULL, FK → sales_product(code)    | Product this price applies to            |
| sales_fob_name      | TEXT        | NOT NULL, FK → sales_fob(name)        | FOB delivery point this price applies to |
| sales_customer_group_name | TEXT  | FK → sales_customer_group(name), nullable | |
| sales_customer_name | TEXT        | FK → sales_customer(name), nullable   | |
...
|--------|------|-------------|-------------|
| org_id | TEXT | NOT NULL, FK → org(id) | Owning organization for RLS filtering |
| sales_customer_group_name | TEXT | FK → sales_customer_group(name), nullable | Cascades to sales_po.sales_customer_group_name when an order is created for this customer |
| sales_fob_name | TEXT | FK → sales_fob(name), nullable | Default FOB delivery point; cascades to sales_po.sales_fob_name when an order is created for this customer |
...
| sales_customer_group_name | TEXT | FK → sales_customer_group(name), nullable | Auto-set from sales_customer.sales_customer_group_name; read-only |
| sales_customer_name | TEXT | NOT NULL, FK → sales_customer(name) | |
| sales_fob_name | TEXT | FK → sales_fob(name), nullable | Auto-set from sales_customer.sales_fob_name; read-only |
...
| approved_by | TEXT | FK → hr_employee(name), nullable | |
| qb_uploaded_at | TIMESTAMPTZ | nullable | |
| qb_uploaded_by | TEXT | FK → hr_employee(name), nullable | |
...
| sales_product_id | TEXT | NOT NULL, FK → sales_product(code) | |
...
| sales_container_type_name | TEXT | FK → sales_container_type(name), nullable | Container type used for shipping; set during containerization |
...
| id | UUID | PK, auto-generated | |
| org_id | TEXT | NOT NULL, FK → org(id) | |
| sales_crm_store_name | TEXT | NOT NULL, FK → sales_crm_store(name) | |
| visit_date | DATE | NOT NULL | |
| notes | TEXT | nullable | |
| visited_by | TEXT | FK → hr_employee(name), nullable | |
...
| org_id | TEXT | NOT NULL, FK → org(id) | |
| sales_crm_store_visit_id | UUID | NOT NULL, FK → sales_crm_store_visit(id) | |
| sales_product_id | TEXT | FK → sales_product(code), nullable | |
| sales_crm_external_product_name | TEXT | FK → sales_crm_external_product(name), nullable | |
...
CHECK constraint: exactly one of sales_product_id or sales_crm_external_product_name must be set.
...
| org_id | TEXT | NOT NULL, FK → org(id) | |
| farm_name | TEXT | FK → org_farm(name), nullable | Derived from the Farm column in the sheet (e.g. "Cuke" → cuke, "Lettuce" → lettuce) |
...
Indexes on `org_id`, `farm_name`, `invoice_date`, `invoice_number`, `customer_name`.
