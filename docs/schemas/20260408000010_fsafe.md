...
| farm_name                | TEXT         | FK → org_farm(name), nullable        | Optional farm scope; null if the test applies to all farms |
...
| fsafe_lab_name                | TEXT         | FK → fsafe_lab(name), nullable          | Pre-filled from fsafe_test_hold.fsafe_lab_name for test-and-hold results; editable |
| fsafe_lab_test_name           | TEXT         | NOT NULL, FK → fsafe_lab_test(id)     | |
...
| sampled_by                  | TEXT         | FK → hr_employee(name), nullable        | |
...
| sales_customer_group_name| TEXT         | FK → sales_customer_group(name), nullable  | Pre-filled from sales_customer.sales_customer_group_name; editable |
| sales_customer_name      | TEXT         | FK → sales_customer(name), nullable        | Pre-filled from the linked sales_po customer; editable |
| fsafe_lab_name           | TEXT         | FK → fsafe_lab(name), nullable             | |
...
| org_id             | TEXT | NOT NULL, FK → org(id)                   | Owning organization for RLS filtering    |
| farm_name            | TEXT | NOT NULL, FK → org_farm(name)                  | Farm this record belongs to; inherited from parent test-and-hold |
