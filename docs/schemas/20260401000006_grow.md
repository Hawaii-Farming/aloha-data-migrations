...
| grow_variety_id | TEXT | NOT NULL, FK → grow_variety(id) | Primary variety planted in this row. If grow_variety_id_2 is null, this variety fills the entire row (num_bags = org_site_gh_row.num_bags_capacity). If split, this variety occupies 50 percent (num_bags = capacity / 2) |
| grow_variety_id_2 | TEXT | FK → grow_variety(id), nullable | Second variety when the row is split. Row is split 50/50: each variety occupies capacity/2 bags. Null for non-split rows |
